...
power_df |>
  ...
  labs(
    ...
    x = "True Mean (μ)",
    y = "Power (Proportion Rejecting H₀)"
  ) +
...
bias_df |>
  ...
  geom_line(aes(y = mean_est_sig, color = "Rejected H₀ only"), linewidth = 1) +
  geom_point(aes(y = mean_est_sig, color = "Rejected H₀ only"), size = 2) +
  labs(
